...
type SectionKey = "customers" | "value" | "revenue" | "activities" | "resources"

const sections: { key: SectionKey; title: string; description: string }[] = [
  {
    key: "customers",
    title: "Your Customers",
    description: "Get specific about who you're serving and what real pain they're paying to solve.",
  },
  {
    key: "value",
    title: "Your Value Proposition",
    description: "Define the transformation you deliver and why your offer wins in a noisy market.",
  },
  {
    key: "revenue",
    title: "Revenue Streams",
    description: "Pressure-test how money flows into the business now — and what grows later.",
  },
  {
    key: "activities",
    title: "Key Activities",
    description: "Find the few actions that actually create momentum, not just busywork.",
  },
  {
    key: "resources",
    title: "Key Resources & Partners",
    description: "Spot what you're leaning on, what's fragile, and where hidden leverage lives.",
  },
]
  ...
  return (
    <div className="space-y-2 text-sm leading-7 text-slate-100">
      {lines.map((line, idx) => {
        ...
        if (line.startsWith("### ")) return <h4 key={idx} className="text-lg font-semibold text-white">{line.replace("### ", "")}</h4>
        if (line.startsWith("## ")) return <h3 key={idx} className="text-xl font-semibold text-white">{line.replace("## ", "")}</h3>
        if (line.startsWith("# ")) return <h2 key={idx} className="text-2xl font-semibold text-white">{line.replace("# ", "")}</h2>
        if (line.match(/^[-*] /)) return <li key={idx} className="ml-5 list-disc">{line.replace(/^[-*] /, "")}</li>
...
export default function BusinessModelBlueprintPage() {
  ...
  const [value, setValue] = useState({ product: "", outcome: "", differentiation: "" })
  ...
  const [activities, setActivities] = useState({ tasks: "", topActivity: "", categories: [] as string[] })
  const [resources, setResources] = useState({ reliedOn: "", partners: "", missing: "" })
  ...
  const completion = useMemo(() => Object.values(insights).filter(Boolean).length, [insights])
  const activeStep = sections.findIndex((section) => section.key === active) + 1
  ...
  return (
    <main className="min-h-screen bg-[#0B0D1A] text-white pt-24 pb-16">
      <div className="mx-auto max-w-6xl px-4 md:px-8 print:max-w-full print:px-0">
        <Link href="/launch-pad/tools" className="mb-6 inline-flex items-center gap-2 text-sm text-slate-300 hover:text-white print:hidden">
        ...
        <section className="mt-8 rounded-2xl border border-white/10 bg-[#111426] p-6 md:p-8">
          <h2 className="text-2xl font-semibold">{sections.find((s) => s.key === active)?.title}</h2>
          <p className="mt-2 text-slate-300">{sections.find((s) => s.key === active)?.description}</p>

          <div className="mt-6 space-y-4">
            {active === "customers" && (
              <>
                <input value={customers.targetCustomer} onChange={(e) => setCustomers((p) => ({ ...p, targetCustomer: e.target.value }))} placeholder="Solo founders building their first digital product business" className="w-full rounded-lg border border-white/10 bg-[#0d1120] px-4 py-3" />
            ...
            {active === "value" && (
              <>
                <textarea value={value.product} onChange={(e) => setValue((p) => ({ ...p, product: e.target.value }))} placeholder="A suite of Notion templates that help solo founders launch their business in 30 days" className="h-28 w-full rounded-lg border border-white/10 bg-[#0d1120] px-4 py-3" />
                <input value={value.outcome} onChange={(e) => setValue((p) => ({ ...p, outcome: e.target.value }))} placeholder="A fully structured business ready to scale" className="w-full rounded-lg border border-white/10 bg-[#0d1120] px-4 py-3" />
                <input value={value.differentiation} onChange={(e) => setValue((p) => ({ ...p, differentiation: e.target.value }))} placeholder="Built specifically for non-technical founders, not corporate teams" className="w-full rounded-lg border border-white/10 bg-[#0d1120] px-4 py-3" />
              </>
            )}
            ...
            {active === "revenue" && (
              <>
                ...
                <input value={revenue.primaryOffer} onChange={(e) => setRevenue((p) => ({ ...p, primaryOffer: e.target.value }))} placeholder="$27 Notion template bundle" className="w-full rounded-lg border border-white/10 bg-[#0d1120] px-4 py-3" />
                <textarea value={revenue.extraStreams} onChange={(e) => setRevenue((p) => ({ ...p, extraStreams: e.target.value }))} placeholder="Describe any other current or planned revenue streams" className="h-24 w-full rounded-lg border border-white/10 bg-[#0d1120] px-4 py-3" />
                <input type="number" value={revenue.monthlyGoal} onChange={(e) => setRevenue((p) => ({ ...p, monthlyGoal: e.target.value }))} placeholder="Approximate monthly revenue goal (USD)" className="w-full rounded-lg border border-white/10 bg-[#0d1120] px-4 py-3" />
              </>
            )}

            {active === "activities" && (
              <>
                <textarea value={activities.tasks} onChange={(e) => setActivities((p) => ({ ...p, tasks: e.target.value }))} placeholder="Creating content, building templates, managing Etsy shop, growing email list" className="h-28 w-full rounded-lg border border-white/10 bg-[#0d1120] px-4 py-3" />
            ...
            {active === "resources" && (
              <>
                <textarea value={resources.reliedOn} onChange={(e) => setResources((p) => ({ ...p, reliedOn: e.target.value }))} placeholder="Notion, Canva, email list, social media, AI tools" className="h-28 w-full rounded-lg border border-white/10 bg-[#0d1120] px-4 py-3" />
                <input value={resources.partners} onChange={(e) => setResources((p) => ({ ...p, partners: e.target.value }))} placeholder="Etsy, Gumroad, Instagram" className="w-full rounded-lg border border-white/10 bg-[#0d1120] px-4 py-3" />
                <textarea value={resources.missing} onChange={(e) => setResources((p) => ({ ...p, missing: e.target.value }))} placeholder="Consistent traffic, a larger audience" className="h-24 w-full rounded-lg border border-white/10 bg-[#0d1120] px-4 py-3" />
              </>
            )}
          </div>
        ...
        <section className="mt-8 rounded-2xl border border-white/10 bg-[#111426] p-6 md:p-8">
          <h3 className="text-2xl font-semibold">Full Business Model Blueprint</h3>
          <p className="mt-2 text-slate-300">Generate a complete, strategic snapshot from every section you've filled in.</p>

          {!hasAnyInput && !fullBlueprint && (
            <p className="mt-4 rounded-lg border border-dashed border-white/20 bg-white/5 p-4 text-sm text-slate-300">Fill in at least one section to generate your Blueprint.</p>
          )}
              ...
              {fullLoading ? (
                ...
                <>
                  <div className="mb-5 flex flex-wrap items-center gap-3 print:hidden">
                    <button onClick={() => copyText(fullBlueprint)} className="inline-flex items-center gap-1 rounded-md border border-white/20 px-3 py-1.5 text-xs text-slate-200 hover:bg-white/10">
                      <Copy className="h-3.5 w-3.5" /> Copy All
                    </button>
                    <button onClick={() => window.print()} className="rounded-md border border-white/20 px-3 py-1.5 text-xs text-slate-200 hover:bg-white/10">
                      Download as PDF
                    </button>
                  </div>
