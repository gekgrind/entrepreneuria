...
function avatarForIndex(index: number) {
  ...
  if (index % 3 === 1) {
    return (
      <div className="h-14 w-14 bg-gradient-to-br from-[#3DB6C6] via-[#77D8D6] to-[#0EA5A8] [clip-path:polygon(50%_0%,100%_35%,82%_100%,18%_100%,0%_35%)] shadow-[0_0_30px_rgba(61,182,198,0.3)]" />
    );
  }

  return (
    <div className="h-14 w-14 rounded-[35%_65%_55%_45%/40%_45%_55%_60%] bg-gradient-to-br from-[#E0B15B] via-[#F4D089] to-[#FFF1B8] shadow-[0_0_30px_rgba(224,177,91,0.3)]" />
  );
}
...
function PersonaCardContent({
  ...
  state: PersonaState;
  index: number;
  setPersonaStates: React.Dispatch<React.SetStateAction<PersonaState[]>>;
  copyText: (text: string) => void;
  downloadAsPdf: (title: string, content: string) => void;
}) {
  ...
  const sections: Array<
    [string, keyof PersonaState["openSections"], string[]]
  > = [
    ["🎯 Core Goals", "goals", persona.goals],
    ["😤 Pain Points & Frustrations", "painPoints", persona.painPoints],
    ["💡 Motivations & Drivers", "motivations", persona.motivations],
    ["🛒 Purchase Behavior", "purchaseBehavior", persona.purchaseBehavior],
    ["🚧 Objections & Hesitations", "objections", persona.objections],
    ["📱 Preferred Channels & Content", "channels", persona.channels],
    ["💬 Voice & Language", "voiceSamples", persona.voiceSamples],
  ];
    ...
    <>
      <div className="mb-4 flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-2xl font-serif">{persona.name}</h3>
            <p className="text-sm text-white/70">{persona.archetypeTitle}</p>
          </div>
        </div>
      </div>
      ...
      {sections.map(([label, key, items]) => (
        <div
          key={String(key)}
          className="mb-3 overflow-hidden rounded-xl border border-white/10 bg-white/[0.03]"
        >
          <button
          ...
          >
            ...
            <ChevronDown
              className={cn(
                "h-4 w-4 transition",
                state.openSections[String(key)] ? "rotate-180" : "",
              )}
            />
          </button>
                ...
                <ul className="list-disc space-y-1 pl-4">
                  {items.map((item, i) => (
                    <li
                      key={i}
                      className={
                        key === "voiceSamples" ? "italic text-white/70" : ""
                      }
                    >
                      {key === "voiceSamples" ? `"${item}"` : item}
                    </li>
                  ))}
                </ul>
              </motion.div>
        ...
        <AnimatePresence initial={false}>
          {state.openSections.conversionRecommendation && (
            <motion.p
              ...
              className="px-4 pb-3 text-sm text-white/80"
            >
              {persona.conversionRecommendation || persona.rawText}
            </motion.p>
          )}
        </AnimatePresence>
      </div>
      ...
      <div className="rounded-xl border border-white/10 bg-[#0f142a] p-3">
        ...
        <p className="text-xs text-white/70">{persona.fitScoreReason}</p>
      </div>
      ...
      <div className="mt-3 flex gap-2">
        <Button
          ...
          onClick={() => copyText(JSON.stringify(persona, null, 2))}
        >
...
export default function CustomerPersonaBuilderPage() {
  ...
  async function buildPersonas() {
    ...
    const requests = Array.from({ length: form.personaCount }, (_, i) => i).map(
      async (index) => {
        setPersonaStates((prev) =>
          prev.map((card, idx) =>
            idx === index
              ? { ...card, loadingMessage: "Building your persona..." }
              : card,
          ),
        );
            ...
            prev.map((card, idx) =>
              idx === index
                ? {
                    ...
                    loading: false,
                    error:
                      "Couldn't build this persona. Your customers are worth a retry.",
                  }
                : card,
            ),
          );
    ...
    if (successful.length > 1) {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            data.error || "Could not generate cross persona insights",
          );
        }
    ...
    >
      <div className="mx-auto max-w-7xl space-y-8">
        <section className="rounded-3xl border border-white/10 bg-gradient-to-br from-[#11152b] to-[#0b0d1a] p-6 shadow-[0_20px_80px_rgba(0,0,0,0.35)] md:p-10">
          ...
          <motion.div
            layout
            className="mt-8 rounded-2xl border border-white/10 bg-white/[0.03] p-5 md:p-7"
          >
            <div className="grid gap-8 md:grid-cols-2">
              <div className="space-y-4">
                <h2 className="text-lg font-semibold text-[#ffd4cb]">
                  Your Business & Offer
                </h2>

                <Input
                  placeholder='e.g., "Notion template bundles that help solo founders organize and launch their business"'
                ...
                <Input
                  placeholder='e.g., "Founders feel overwhelmed and don&apos;t know where to start — my templates give them a system"'
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setForm((prev) => ({ ...prev, priceRange: value }))
                  }
                >
                  <SelectTrigger>
              ...
              <div className="space-y-4 border-t border-white/10 pt-8 md:border-l md:border-t-0 md:pl-8 md:pt-0">
                ...
                <Textarea
                  placeholder='e.g., "Early-stage solopreneurs, mostly women 28–42..."'
                ...
                <Input
                  placeholder='e.g., "Launch a profitable business without quitting their job first"'
                  value={form.customerGoal}
                  onChange={(e) =>
                    setForm((prev) => ({
                      ...prev,
                      customerGoal: e.target.value,
                    }))
                  }
                />

                <Input
                  placeholder='e.g., "Instagram, Pinterest, LinkedIn, Reddit r/Entrepreneur"'
                  value={form.onlineChannels}
                  onChange={(e) =>
                    setForm((prev) => ({
                      ...prev,
                      onlineChannels: e.target.value,
                    }))
                  }
                />

                <Select
                  value={String(form.personaCount)}
                  onValueChange={(value) =>
                    setForm((prev) => ({
                      ...prev,
                      personaCount: Number(value),
                    }))
                  }
                >
                  <SelectTrigger>
                ...
                <Textarea
                  placeholder='e.g., "Research communities like r/Entrepreneur..."'
                  value={form.additionalResearch}
                  onChange={(e) =>
                    setForm((prev) => ({
                      ...prev,
                      additionalResearch: e.target.value,
                    }))
                  }
                  className="min-h-24"
                />
            ...
            <div className="mt-8 flex flex-col items-start gap-3">
              ...
              <p className="text-sm text-white/60">
                Personas are AI-generated composites for strategic planning.
                Validate with real customer research whenever possible.
              </p>
            </div>
          </motion.div>
        </section>
        ...
        {!personaStates.length && (
          <section className="rounded-2xl border border-dashed border-white/20 p-10 text-center text-white/60">
            Tell us about your offer and customer above — we&apos;ll handle the
            research.
          </section>
        )}
        ...
        {!!personaStates.length && (
          <section className="space-y-5">
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="font-serif text-3xl">Your Customer Personas</h2>

              <span className="rounded-full border border-white/15 bg-white/5 px-3 py-1 text-xs text-white/70">
                {form.personaCount} personas · AI + Live Research · Generated{" "}
                {generatedAt}
              </span>
            </div>
                  ...
                  {state.loading ? (
                    <div className="animate-pulse space-y-4">
                      ...
                      {Array.from({ length: 6 }).map((_, i) => (
                        <div
                          key={i}
                          className="h-12 rounded-xl bg-[#f4a267]/10"
                        />
                      ))}
                      <p className="text-sm text-[#ffd4cb]">
                    ...
                    <PersonaCardContent
                      state={state}
                      index={index}
                      setPersonaStates={setPersonaStates}
                      copyText={copyText}
          ...
          >
            ...
            {crossLoading ? (
              <p className="text-white/70">
                Connecting the dots across your audience...
              </p>
            ) : (
        ...
        {completedPersonas.length > 0 && !globalLoading && (
          <section className="space-y-4 rounded-2xl border border-white/10 bg-white/[0.03] p-6">
            ...
            {strategyLoading && (
              <p className="text-white/70">
                Building your founder-ready strategy report...
              </p>
            )}
              ...
              <>
                ...
                <div className="flex gap-2">
                  <Button
                    variant="outline"
                    className="border-white/20 bg-transparent"
                    onClick={() => copyText(strategyReport)}
                  >
                    <Copy className="mr-2 h-4 w-4" />
                    Copy
                  </Button>
                  ...
                  <Button
                    ...
                    className="border-white/20 bg-transparent"
                    onClick={() =>
                      downloadAsPdf("Persona Strategy Report", strategyReport)
                    }
                  >
                    <Download className="mr-2 h-4 w-4" />
                    Download as PDF
