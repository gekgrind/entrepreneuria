...
export function UserAccountMenu({ fullName, email, avatarUrl }: UserAccountMenuProps) {
  const label = fullName?.trim() || "Account";
  const secondaryLabel = email.trim() || "No email";
      ...
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" className="h-auto w-full justify-between px-2 py-2">
          <div className="flex min-w-0 items-center gap-2">
            <UserAvatar fullName={fullName} email={email} avatarUrl={avatarUrl} className="size-8" />
